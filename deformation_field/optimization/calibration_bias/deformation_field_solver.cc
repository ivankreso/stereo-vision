...
DeformationFieldSolver::DeformationFieldSolver(const double* const cam_intr, int bin_rows, int bin_cols,
                                               int img_rows, int img_cols, double loss_scale,
                                               std::string save_filepath) :
                                               bin_rows_(bin_rows), bin_cols_(bin_cols), img_rows_(img_rows),
                                               img_cols_(img_cols), loss_scale_(loss_scale),
                                               save_filepath_(save_filepath)
{
...
void DeformationFieldSolver::Solve()
{
  ...
  int central_col = bin_cols_ / 2;
  for (int r = 0; r < bin_rows_; r++) {
    int cbin = r * bin_cols_ + central_col;
    if (left_num_points_[cbin] < min_points_ || right_num_points_[cbin] < min_points_)
      throw 1;
    for (int c = central_col - 1; c >= 0; c--) {
      int bin_num = r * bin_cols_ + c;
      if (left_num_points_[bin_num] < min_points_) {
        left_dx_[bin_num] = left_dx_[bin_num + 1];
        left_dy_[bin_num] = left_dy_[bin_num + 1];
      }
      if (right_num_points_[bin_num] < min_points_) {
        right_dx_[bin_num] = right_dx_[bin_num + 1];
        right_dy_[bin_num] = right_dy_[bin_num + 1];
      }
    }
    for (int c = central_col + 1; c < bin_cols_; c++) {
      int bin_num = r * bin_cols_ + c;
      if (left_num_points_[bin_num] < min_points_) {
        left_dx_[bin_num] = left_dx_[bin_num - 1];
        left_dy_[bin_num] = left_dy_[bin_num - 1];
  ...
  double scale = 1.3;
  int cell_cols = scale * 1241 / 15;
  int cell_rows = scale * 376 / 5;
  ...
  mat_file << "robust_loss_scale" << loss_scale_;
    ...
    ceres::Problem& ceres_problem)
{
  ...
  const int min_points = min_points_;
  if (left_num_points_[lp_bin] < min_points || left_num_points_[lc_bin] < min_points ||
      right_num_points_[rp_bin] < min_points ||  right_num_points_[rc_bin] < min_points)
    return;
  ...
  //ceres::LossFunction* loss_func = new ceres::CauchyLoss(2.0);
