                         ...
                         const std::string& deformation_field_path)
{
  ...
  egomotion::EgomotionBase* viso = nullptr;
  ...
  // else it is KITTI
    ...
    // try 6 and 4
    //error_thr = 5.0; // remove moving cars with 5.0 - works worssse...
    error_thr = 8.0;
  }
  ...
  for (unsigned i = start_frame + 2; i < end_frame; i += 2) {
    ...
    gt_filtering_passed = track::EvalHelper::FilterOutliersWithGroundtruth(*stereo_tracker,
                           cam_params, gt_world_motion[motion_idx], error_thr); // the best KITTI thresh - 8.0

    //cv::Mat img_lp, img_rp, img_lc, img_rc;
    //track::EvalHelper::DrawTracksWithBigErrors(*stereo_tracker, cam_params, gt_world_motion[motion_idx],
    //                              2.0, 7.0, -1, img_left_prev, img_right_prev, img_left, img_right,
