#include "stereo_tracker_freak.h"
...
void StereoTrackerFREAK::DrawKeypoints(const cv::Mat& img, const std::vector<cv::KeyPoint>& points,
...
void StereoTrackerFREAK::DrawFullTracks() const {
...
void StereoTrackerFREAK::DrawMatches() const {
  cv::Mat disp_img;
  cv::cvtColor(img_lp_, disp_img, cv::COLOR_GRAY2BGR);
  for (size_t i = 0; i < tracks_lp_.size(); i++) {
    if (age_[i] < 1) continue;
...
void StereoTrackerFREAK::DrawStereo() const {
  cv::Mat disp_img;
...
StereoTrackerFREAK::StereoTrackerFREAK(
    ...
    bool freak_orientation, float freak_size) :
      ...
      left_detector_(left_detector), right_detector_(right_detector) {
  if (max_epipolar_diff != 0)
    DEB << "Urectified case detected: max epipolar difference is not zero!";
  ...
  //descriptor_ = cv::xfeatures2d::FREAK::create(true, true, 22.0, 4);
  //descriptor_ = cv::xfeatures2d::FREAK::create(true, false, 11.0, 1);
  descriptor_ = cv::xfeatures2d::FREAK::create(freak_orientation, false, freak_size, 1);

  //matcher_ = cv::cuda::DescriptorMatcher::createBFMatcher(cv::NORM_HAMMING);
  //bool cross_check = false;
  //matcher_ = new cv::BFMatcher(cv::NORM_HAMMING, cross_check);
  matcher_ = cv::cuda::DescriptorMatcher::createBFMatcher(cv::NORM_HAMMING);
}

void StereoTrackerFREAK::init(const cv::Mat& img_left, const cv::Mat& img_right) {
  std::vector<cv::ocl::PlatformInfo> info;
  cv::ocl::getPlatfomsInfo(info);
  //cv::ocl::setUseOpenCL(false);
  //for (auto i : info) {
  //  std::cout << i.deviceNumber() << "\n";
  //  std::cout << i.name() << "\n";
  ...
  descriptors_lp_.create(max_tracks_, descriptor_->descriptorSize(), descriptor_->descriptorType());
  prev_stereo_dists_.assign(max_tracks_, 0);
  curr_stereo_dists_.assign(max_tracks_, 0);
  mono_dists_.assign(max_tracks_, 0);

  img_left.copyTo(img_lc_);
  img_right.copyTo(img_rc_);

  std::vector<cv::KeyPoint> points_left, points_right;
  left_detector_->detect(img_left, points_left);
  right_detector_->detect(img_right, points_right);
  //for (auto& i : points_left)
  //  std::cout << i.angle << "\n" << i.size << "\n" << i.octave << "\n" << i.response << "\n\n";

  cv::Mat desc_left, desc_right;
  descriptor_->compute(img_left, points_left, desc_left);
  descriptor_->compute(img_right, points_right, desc_right);
  cv::cuda::GpuMat gpu_desc_left, gpu_desc_right, mask;
  gpu_desc_left.upload(desc_left);
  gpu_desc_right.upload(desc_right);
  ...
  std::vector<std::vector<cv::DMatch>> matches;
  matcher_->radiusMatch(gpu_desc_left, gpu_desc_right, matches, maxdist_stereo_, mask);
  ...
  for (size_t j = 0; j < matches.size(); j++) {
    ...
    gpu_desc_left.row(m.queryIdx).copyTo(descriptors_lp_.row(i));
    age_[i] = 0;
    curr_stereo_dists_[i] = std::round(m.distance);
    i++;
  }
  if (verbose_) {
...
void StereoTrackerFREAK::track(const cv::Mat& img_left, const cv::Mat& img_right) {
  ...
  std::swap(prev_stereo_dists_, curr_stereo_dists_);

  std::vector<cv::KeyPoint> points_left, points_right;
  //cv::cuda::GpuMat gpu_points_left, gpu_points_right;
  cv::Mat cpu_descriptors_left, cpu_descriptors_right;
  left_detector_->detect(img_left, points_left);
  right_detector_->detect(img_right, points_right);
  descriptor_->compute(img_left, points_left, cpu_descriptors_left);
  descriptor_->compute(img_right, points_right, cpu_descriptors_right);
  std::vector<std::vector<cv::DMatch>> matches;
  ...
  descriptors_left.upload(cpu_descriptors_left);
  descriptors_right.upload(cpu_descriptors_right);
  ...
  std::vector<float> stereo_dists(points_left.size(), -1);
  ...
  for (size_t i = 0; i < stereo_matches_compact.size(); i++) {
    if (stereo_matches_compact[i].distance <= maxdist_stereo_) {
      ...
      stereo_dists[left_idx] = stereo_matches_compact[i].distance;
    }
  }
  ...
  for (const auto& m : temp_matches_compact) {
    ...
    if (stereo_matches[curr_idx] >= 0 && m.distance <= maxdist_temp_) {
      ...
      curr_stereo_dists_[m.queryIdx] = stereo_dists[curr_idx];
      mono_dists_[m.queryIdx] = std::round(m.distance);
    }
    ...
    if (used_matches[i] == false && right_idx >= 0) {
      ...
      while (cnt < age_.size()) {
        if (age_[cnt] < 0) {
          ...
          age_[cnt] = 0;
          curr_stereo_dists_[cnt] = std::round(stereo_dists[i]);
          cnt++;
          break;
        }
        cnt++;
...
FeatureInfo StereoTrackerFREAK::featureLeft(int i) const {
  FeatureInfo feat;
  feat.age_ = age_[i];
...
FeatureInfo StereoTrackerFREAK::featureRight(int i) const {
  FeatureInfo feat;
  feat.age_ = age_[i];
...
TrackStats StereoTrackerFREAK::GetTrackStats(int i) const {
  TrackStats stats(-1, -1, -1);
  if (age_[i] >  0) {
    float prev_resp = tracks_lp_[i].response + tracks_rp_[i].response;
    stats.left_response_sum = prev_resp + tracks_lc_[i].response;
    stats.right_response_sum = prev_resp + tracks_rc_[i].response;
    //std::cout << prev_stereo_dists_[i] << " -- " <<  curr_stereo_dists_[i] << " -- "
    //          << mono_dists_[i] << "\n";
    assert(prev_stereo_dists_[i] >= 0 && curr_stereo_dists_[i] >= 0 && mono_dists_[i] >= 0);
    stats.matching_distance = prev_stereo_dists_[i] + curr_stereo_dists_[i] + mono_dists_[i];
  }
  return std::move(stats);
}

void StereoTrackerFREAK::ApplyEpipolarConstraint(
...
void StereoTrackerFREAK::ApplyTemporalConstraint(
