...
typedef struct  {
  ...
  */
  ...
  /* for the adaptive feature background model */
  double affine_bgmodel_alpha;
  double affine_bgmodel_thsigma;

  /* for the eventual Gaussian window */
  void *img_gauss;  
...
typedef struct  {
  ...
  KLT_locType lambda;
  KLT_locType delta;
  _KLT_FloatImage aff_img_last; //ss
  _KLT_FloatImage aff_img_mask; //ss
  _KLT_FloatImage aff_img_mu; //ss
  _KLT_FloatImage aff_img_sigmaSqr; //ss
  float residual_aff;//ss
  float residual_trans;//ss
  float lambda2;//ss
  int pyrind;//ss
  float magfactor[1];//ss
  int indexMagfactor;
  int id;
...
void KLTInitFeature(KLT_Feature);
void KLTFreeFeature(KLT_Feature);



/* Create */
KLT_TrackingContext KLTCreateTrackingContext(
  int translation_window_width, int translation_window_height,
  int affine_window_width, int affine_window_height);
