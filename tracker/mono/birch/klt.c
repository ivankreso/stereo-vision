...
//static const int window_size = 7;//ss
static const int min_eigenvalue = 1;
static const float min_determinant = 10000000;//ss
static const float min_displacement = 0.1f;
static const int max_iterations = 20;//ss
static const float max_residue = 20;//ss
static const float grad_sigma = 1.0f;
static const float smooth_sigma_fact = 0.1f;
static const float pyramid_sigma_fact = 0.8f;//
static const KLT_BOOL sequentialMode = FALSE;
/* for affine mapping*/
//static const int affineConsistencyCheck = -1;//ss
//static const int affine_window_size = 15;
static const int affine_max_iterations = 10;
static const float affine_max_residue = 15.0f;//ss
static const float affine_min_displacement = 0.02f;
static const float affine_max_displacement_differ = 2.5f;//ss
...
static void makeGauss(float *img, int nrows, int ncols){
  int i,j;
  double sigma=ncols/4.0;
  int w=ncols/2;
  int h=nrows/2;
  float *p=img;
  float total=0;
  for (i = -w ; i <= w ; ++i) {
    for (j = -h ; j <= h ; ++j) {
      *p=(float) exp(-(i*i+j*j) / (2*sigma*sigma));
      total+= *p;
      p++;
    }
  }
...
void KLTInitFeature(KLT_Feature f){
  f->x     = -1;
  f->y     = -1;
  f->xpred = -1;
  f->ypred = -1;
...
void KLTFreeFeature(KLT_Feature f){
  _KLTFreeFloatImage(f->aff_img);
  _KLTFreeFloatImage(f->aff_img_gradx);
  _KLTFreeFloatImage(f->aff_img_grady);
  _KLTFreeFloatImage(f->aff_img_last);
  _KLTFreeFloatImage(f->aff_img_mask);
  _KLTFreeFloatImage(f->aff_img_mu);
  _KLTFreeFloatImage(f->aff_img_sigmaSqr);
}
...
KLT_TrackingContext KLTCreateTrackingContext(
  int myTranslationWindowWidth, int myTranslationWindowHeight,
  int myAffineWindowWidth, int myAffineWindowHeight)
{
  ...
  tc->mindist = mindist;
  tc->window_width = myTranslationWindowWidth;
  tc->window_height = myTranslationWindowHeight;
  ...
  tc->affine_window_width = myAffineWindowWidth;
  tc->affine_window_height = myAffineWindowHeight;
  ...
  tc->affine_bgmodel_alpha   = 0.01;
  tc->affine_bgmodel_thsigma = 12.0;
  ...
  tc->img_gauss = _KLTCreateFloatImage(
    tc->affine_window_width, tc->affine_window_height);
  makeGauss((float*)tc->img_gauss,
    tc->affine_window_width, tc->affine_window_height);
  fillBuf((float*)tc->img_gauss, 
    tc->affine_window_width*tc->affine_window_height, 1);
  ...
  int nFeatures)
{
  ...
  for (i = 0 ; i < nFeatures ; i++){
    ...
    KLTInitFeature(fl->feature[i]);
  }

  /* Return feature list */
  return(fl);
}
KLT_FeatureList KLTResizeFeatureList(
  KLT_FeatureList flOld,
  int nFeatNew)
{
  int nFeatOld=flOld->nFeatures; 
  KLT_FeatureList flNew=KLTCreateFeatureList(nFeatNew);
  int nFeatCpy=nFeatOld;
  if (nFeatNew<nFeatOld)
    nFeatCpy=nFeatNew;

  //copy features
  memcpy(flNew->feature+nFeatNew, 
         flOld->feature+nFeatOld, 
         nFeatCpy*sizeof(KLT_FeatureRec));
  free(flOld);
  return flNew;
}
/*********************************************************************
  ...
  KLT_FeatureList fl)
{
  ...
  for (indx = 0 ; indx < fl->nFeatures ; indx++)  {
    KLTFreeFeature(fl->feature[indx]);
  }
