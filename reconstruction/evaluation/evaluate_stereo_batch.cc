...
bool Eval(std::string result_dir, std::string result_name, std::vector<Errors>& results) {

  // ground truth and result directories
  std::string gt_folder = "/home/kivan/Projects/datasets/KITTI/dense_stereo/training/";
  string gt_noc_dir = gt_folder + "disp_noc";
  string gt_occ_dir = gt_folder + "disp_occ";
  ...
  kitti_errors.name = result_name;
  results.push_back(kitti_errors);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  std::string stats_file = directory + "/../stats.txt";
  std::ofstream stats_fp(stats_file);
  for(size_t i = 0; i < dirs.size(); i++) {
    std::cout << "\nEvaluating: " + directory + "/" + result_names[i] << '\n';
    Eval(dirs[i], result_names[i], results);
  }
  std::sort(std::begin(results), std::end(results),
    [](Errors const &t1, Errors const &t2) { return t1.out_noc < t2.out_noc; });

  for(size_t i = 0; i < results.size(); i++)
    stats_fp << std::left << std::setfill(' ') << std::setw(40) << std::fixed << std::setprecision(2)
             << results[i].name << results[i].out_noc << "\t" << results[i].out_all << "\t" << results[i].avg_noc
             << "\t" << results[i].avg_all << "\t" << results[i].density << "\n";
  ...
  //std::string fpath = "/home/kivan/Projects/datasets/results/dense_stereo/stats.txt";
