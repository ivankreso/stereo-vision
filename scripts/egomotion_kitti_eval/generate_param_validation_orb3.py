...
patch_size = [11, 15, 17]
scale_factor = [1.03, 1.05, 1.1, 1.2]
num_levels = [1, 3, 4, 6, 8]
max_dist = [55, 60, 65, 70]

fp_runscript = open("/home/kivan/source/cv-stereo/scripts/egomotion_kitti_eval/run_validation_orb3.sh", 'w')
    ...
    for j in range(len(num_levels)):
        for k in range(len(scale_factor)):
            for l in range(len(max_dist)):
                cnt += 1
                filepath = "/home/kivan/source/cv-stereo/config_files/experiments/kitti/validation/orb/validation_orb3_" + str(cnt) + ".txt"
                print(filepath)
                fp = open(filepath, 'w')
