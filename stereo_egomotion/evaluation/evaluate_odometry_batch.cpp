...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  std::cout << ": max dist = " << dist.back() << " meters\n";
...
bool eval(int start, int end, const std::string& directory,
          const std::string& result_name, std::vector<ResultTuple>& results, bool remove_partial)
{
  // ground truth and result directories
  //string gt_dir         = "data/odometry/poses/";
  string gt_dir         = "../../data/GT/KITTI/poses/";
  ...
  //for (int32_t i=0; i<11; i++) {
    ...
    if(poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
      if(remove_partial) {
        std::cout << "[Warn]: Skipping experiment... no data for track: " << file_name << '\n';;
        return false;
      }
      else {
        std::cout << "[Warn]: Ignoring... no data for track: " << file_name << '\n';;
        continue;
      }
    }
...
int main (int32_t argc,char *argv[]) {
  if(argc != 6) {
    cout << "Usage: ./eval_odometry start_num end_num remove_partial(0-no, 1-yes) dir_name out_filename" << endl;
    return 1;
  }
  ...
  bool remove_partial = std::stoi(argv[3]);
  ...
  for(size_t i = 0; i < dirs.size(); i++) {
    ...
    eval(start, end, directory, result_names[i], results, remove_partial);
  }
  std::sort(std::begin(results), std::end(results),
    [](ResultTuple const &t1, ResultTuple const &t2) {
  ...
  for(size_t i = 0; i < results.size(); i++) {
    ...
    stats_fp << std::left << std::setfill(' ') << std::setw(50) << std::get<0>(results[i])
             << std::fixed << std::setprecision(2) << std::get<1>(results[i]) << "\t"
             << std::setprecision(4) << std::get<2>(results[i]) << "\n";
