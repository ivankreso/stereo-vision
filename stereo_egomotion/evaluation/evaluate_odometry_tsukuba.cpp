...
bool eval(int start, int end, const std::string& directory, const std::string& gt_file,
          const std::string& result_name, std::vector<ResultTuple>& results)
{
  ...
  //for (int32_t i=0; i<11; i++) {
    ...
    std::vector<Matrix> poses_result = loadPoses(result_dir + file_name);
   
    // check for errors
    if (poses_gt.size() == 0 || poses_result.size() != poses_gt.size()) {
      std::cout << "Error!\nGT size = " << poses_gt.size() << "\nVO size = " << poses_result.size() << "\n";
      return false;
    }
...
int main (int32_t argc,char *argv[]) {
  if(argc != 6) {
    cout << "Usage: ./eval_odometry gt_filepath start_num end_num dir_name out_filename" << endl;
    return 1;
  }
  ...
  //std::string gt_file = "/home/kivan/Projects/cv-stereo/data/GT/Tsukuba/tsukuba_gt_crop.txt";
  ...
  for(size_t i = 0; i < dirs.size(); i++) {
    ...
    bool success = eval(start, end, directory, gt_file, result_names[i], results);
    if(!success)
      std::cout << "error in: " << directory + result_names[i] << "\n";
