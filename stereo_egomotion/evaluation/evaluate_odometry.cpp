...
bool eval(int start, int end, string result_sha,Mail* mail) {
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=4 && argc!=6) {
    cout << "Usage: ./eval_odometry start_num end_num result_sha [user_sha email]" << endl;
    return 1;
  }
  ...
  string result_sha = argv[3];

  // init notification mail
  Mail *mail;
  if (argc==4) mail = new Mail(argv[5]);
  else         mail = new Mail();
  mail->msg("Thank you for participating in our evaluation!");

  // run evaluation
  bool success = eval(start, end+1, result_sha, mail);
