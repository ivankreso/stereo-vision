                         ...
                         const std::string& gt_filepath)
{
  ...
  if(use_ba) {
    ...
    sba->setCameraIntrinsics(cam_params);
  }
  ...
  for(unsigned i = start_frame + 2; i < end_frame; i+=(2)) {
    ...
    if(viso->process(libviso_tracks)) {
      ...
      ////if(using_kitti)
      ////  filter_bad = true;
      //int bad_tracks = track::EvalHelper::CountBadTracks(*stereo_tracker, cam_params, Rt_gt, 1.0, filter_bad);
      ...
      //if(frame_num >= 1160) {
      //      // draw tracks
        //cv::cvtColor(img_left_prev, disp_camera_left, COLOR_GRAY2RGB);
        //cv::cvtColor(img_right_prev, disp_camera_right, COLOR_GRAY2RGB);
        //FeatureHelper::drawStereoTracks(*stereo_tracker, disp_camera_left, disp_camera_right);
      ...
      if(use_ba) {
        ...
        // TODO
        sba->updateTracks(*stereo_tracker, Rt_inv);
