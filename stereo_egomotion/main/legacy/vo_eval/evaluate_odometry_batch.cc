...
float lengths[] = {100,200,300,400,500,600,700,800};      // for KITTI
//float lengths[] = {400,800,1200,1600,2000,2400,2800,3200};  // for Tsukuba
...
void saveStats (vector<errors> err,string dir) {
  ...
  float num = err.size();
  fprintf(fp,"%f %f\n",t_err/num,r_err/num);
  printf("Trans error: %f\nRot error: %f\n", t_err/num, r_err/num);
  // close file
  fclose(fp);
}

bool eval(string filepath1, string filepath2) {
  ...
  string file_name = "out_file.txt";
  // read ground truth and result poses
  vector<Matrix> poses_gt     = loadPoses(filepath1);
  vector<Matrix> poses_result = loadPoses(filepath2);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // run evaluation
  bool success = eval(filepath1, filepath2);
