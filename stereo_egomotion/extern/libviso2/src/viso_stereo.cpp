...
VisualOdometryStereo::VisualOdometryStereo(parameters params) : param(params), VisualOdometry(params)
{
}

VisualOdometryStereo::VisualOdometryStereo(parameters params, StereoTrackerBase* tracker) :
                                           param(params), matcher(tracker), VisualOdometry(params)
{
}
...
bool VisualOdometryStereo::process(uint8_t *I1, uint8_t *I2, int32_t* dims, bool replace) {
  ...
  throw "Error";
  //cv::Mat img_l(dims[1], dims[0], CV_8U, I1);
  //cv::Mat img_r(dims[1], dims[0], CV_8U, I2);
  //// if not first run -> track
  //if(!first_run_) {
  //  matcher->track(img_l, img_r);
  //}
  //// else init in first run
  //else {
  //  first_run_ = false;
  //  matcher->init(img_l, img_r);
  //  return false;
  //}

  //// enable this only if the tracker doesn't do this already
  //int bad_cnt = FeatureHelper::filterBadTracks(*matcher);
  //std::cout << "[VisualOdometry]: filtered bad tracks cnt: " << bad_cnt << "\n";

  //// without feature stratification - use with orig libviso matcher and with sba
  //std::vector<int> active_tracks;
  //FeatureHelper::TrackerBaseToLibviso(matcher, p_matched, active_tracks);

  //// with feature stratif
  ////FeatureHelper::TrackerBaseToLibvisoUniform(matcher, p_matched, 15, 10, 10, img_l.size(), active_tracks);
  ////FeatureHelper::TrackerBaseToLibvisoStratified(matcher, p_matched, 2, 50, 50, img_l.size());
  ////FeatureHelper::TrackerBaseToLibvisoStratified(matcher, p_matched, param.bucket.max_features,
  ////                                              param.bucket.bucket_width, param.bucket.bucket_height,
  ////                                              img_l.size(), active_tracks);

  //bool status = updateMotion();

  //// can't use with unfirom feature selection..
  //// is it better to kill or not to kill outliers in tracker?
  //std::cout << "Tracks before RANSAC: " << matcher->countActiveTracks() << "\n";
  ////FeatureHelper::filterOutlierTracks(*matcher, active_tracks, inliers, outliers_);
  //FeatureHelper::FilterOutlierTracks(*matcher, active_tracks, inliers);
  //std::cout << "Tracks after RANSAC: " << matcher->countActiveTracks() << "\n";
  //return status;
}
...
vector<double> VisualOdometryStereo::estimateMotion(vector<Matcher::p_match> p_matched) {
  ...
  for (int32_t i=0; i<N; i++) {
    ...
    if(d <= 0.0) {
      cout << "[VisualOdometryStereo] zero/negative disp: " << d << " -> ";
      cout << p_matched[i].u1p << " - " << p_matched[i].u2p << "\n";
      exit(-1);
    }
    //else if(d < 1.0) cout << "[VisualOdometryStereo] small disp: " << d << "\n";
  ...
  printf("[libviso]: RANSAC found most inliers in iter %d / %d\n", best_iter_num, param.ransac_iters);
...
void VisualOdometryStereo::computeResidualsAndJacobian(vector<double> &tr,vector<int32_t> &active) {
  ...
  for (int32_t i=0; i<(int32_t)active.size(); i++) {
    ...
    for (int32_t j=0; j<6; j++) {
      ...
      J[(4*i+0)*6+j] = weight_trans * weight*param.calib.f*(X1cd*Z1c-X1c*Z1cd)/(Z1c*Z1c); // left u'
      J[(4*i+1)*6+j] = weight_trans * weight*param.calib.f*(Y1cd*Z1c-Y1c*Z1cd)/(Z1c*Z1c); // left v'
      J[(4*i+2)*6+j] = weight_trans * weight*param.calib.f*(X1cd*Z1c-X2c*Z1cd)/(Z1c*Z1c); // right u'
      J[(4*i+3)*6+j] = weight_trans * weight*param.calib.f*(Y1cd*Z1c-Y1c*Z1cd)/(Z1c*Z1c); // right v'
    }
