...
public:
   ...
   VisualOdometryStereo(parameters params, track::StereoTrackerBase* tracker);
   // deconstructor
   ...
   std::vector<int>  getRansacInliers(std::vector<Matcher::p_match> p_matched);

   using VisualOdometry::process;

   // ADDED
   std::vector<int> getOutliers() { return outliers_; }
   // parameters
   parameters param;
...
private:
   ...
   // ADDED
   track::StereoTrackerBase* matcher = nullptr;    // feature matcher  
